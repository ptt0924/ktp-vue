<template>
    <common-navigation class="task-commit-nav">
    <template v-slot:left><div style="cursor: pointer" @click="back"><i class="el-icon-back" /><span>{{cname}}</span></div></template>
    <template v-slot:center>
      <div class="nav-center">
        <div @click="changeStyle">{{$store.state.role === '2' ? '学生作业' : '提交作业'}}</div>
        <div @click="changeStyle(false)">作业讨论</div>
      </div>
    </template>
  ...
  methods: {
    ...
    changeStyle(res = true) {
      ...
      if (this.styleOpt) {
        ...
        document.querySelector('.nav-center>div:last-child').style.borderBottom = "none"
        document.querySelector('.nav-center>div:last-child').style.borderLeft = "none"
        document.querySelector('.nav-center>div:last-child').style.borderRight = "none"
      } else {
        document.querySelector('.nav-center>div:first-child').style.borderBottom = "none"
        document.querySelector('.nav-center>div:first-child').style.borderLeft = "none"
        document.querySelector('.nav-center>div:first-child').style.borderRight = "none"
