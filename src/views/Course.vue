...
<!--        </div>-->
    ...
    <!-- 编辑课程模态框 -->
    <el-dialog :visible.sync="editCourseDialog" width="660px" top="30vh" v-if="this.$store.state.role === '2'">
...
export default {
  ...
  data() {
    return {
      ...
      bg: "",
      optionState: 0, // 0为课堂互动 1为教师作业 2为学生作业 3为话题
      editCourseObj: {
        courseName: "",
      },
      optionSemester: [
  ...
  methods: {
    ...
    clickOption(val) {
      if (val === this.optionState) {
        return;
      }

      this.optionState = val;
      ...
      switch (val) {
        ...
        // 作业
        case 1:
          val = `/homework/${this.course.cno}`;
          break;
        // 学生作业
        case 2:
          val = `/stuHomework/${this.course.cno}`
          break;
        // 资料
        case 3:
          val = "/topic";
      ...
      // 等同于声明式 <router-link to>
      this.$router.push(val); /*编程式*/
    },
    getCourse() {
      ...
      axios.all([courseConfig, taskConfig])
          .then(axios.spread((res1, res2) => {
            if (res1.data !== undefined && res1.data != null && res2.data !== undefined && res2.data != null) {
              this.course = res1.data
              this.tasks = res2.data
              this.editCourseObj = JSON.parse(JSON.stringify(this.course));
              console.log(this.editCourseObj);
            } else {
              console.log('else');
              this.$message({
                showClose: true,
  ...
  mounted() {
    ...
    this.getCourse()
    this.bg = require(`../assets/theme/${Math.floor(Math.random() * 44) + 1}.png`);
  },
};
</script>
<style scoped>
